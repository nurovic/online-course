...
class Course {
  ...
  async create(req, res) {
    ...
    const result = await cloudinary.uploader.upload(
      ...
      {
        resource_type: "image",
        folder: `${req.body.user_id.full_name}/movie/cover`,
      }
    );
  ...
  findOne(req, res) {
    ...
    CourseService.findOne({ _id: req.params.id })
      .then((course) => {
    ...
    CourseService.update(req.params?.id, req.body)
      ...
      .catch((e) =>
        res
          ...
          .send({ message: "Error occured while upadate course" })
      );
  }
  ...
  async deleteCourse(req, res) {
    if (!req.params?.id) {
      return res
        ...
        .send({ message: "ID information is missing for Delete" });
    }
    ...
    await CourseService.deleteAll({ _id: { $in: req.params?.id } })
      .then((deleteCourse) => {
        if (!deleteCourse) {
          return res
            ...
            .send({ message: "This course not found" });
        }
      ...
      .catch((e) =>
        res
          .status(httpStatus.INTERNAL_SERVER_ERROR)
          .send({ message: "Error occured while delete course" })
      );
  }

  projectList(req, res) {
    ProjectService.list({ course_id: req.params.id })
      .then((projects) => {
        res.status(httpStatus.OK).send(projects);
      })
      .catch((e) =>
        httpStatus.INTERNAL_SERVER_ERROR.send({ error: "error occured" })
      );
  }

  makeComment(req, res) {
    // const { id } = req.params
    CourseService.findOne({ _id: req.params.id }).then((courseTask) => {
      const comment = {
        ...req.body,
      ...
      courseTask
        ...
        .catch((e) =>
          res
            ...
            .send({ error: "Error occured while comment" })
        );
    });
  }

  deleteComment(req, res) {
    CourseService.findOne({ _id: req.params.id }).then((mainCourse) => {
      // if(!mainCourse) return res.status(httpStatus.NOT_FOUND).send({message: "This comment not found"})
      mainCourse.comments = mainCourse.comments.filter((d) => {
        return d._id.toString() != req.params.commentId;
      });
      mainCourse
        .save()
        .then((update) => {
